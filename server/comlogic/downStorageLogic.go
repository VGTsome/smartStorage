...
func SetLight(cabinetName string, light string) {
	...
	retCommand += shelf + " "
	retCommand += box + " "
	retCommand += "30 " + light + " 00 00 00 00 00"
	retCommand = buildWholeCmd(retCommand)
	sendCommand(com, retCommand)
}
...
func InitProduct(cabinetName string, productNum int) {
	...
	retCommand += box + " "
	retCommand += "31 01"
	retCommand += hexAddPreZero(intToHexString(productNum), 1)
	retCommand += " 00 00 00"
	retCommand = buildWholeCmd(retCommand)
	SetLight(cabinetName, "31")
...
func downSetProductInfo(cabinetName string, singleWeight int) {
	...
	retCommand += "31 31 "
	//单重
	retCommand += hexAddPreZero(intToHexString(singleWeight), 4)
	//皮重
	retCommand += hexAddPreZero(intToHexString(0), 4)
	//满盒数量
	retCommand += hexAddPreZero(intToHexString(1), 4)
	retCommand += hexAddPreZero(intToHexString(0), 5)
	retCommand = buildWholeCmd(retCommand)
